...
This document describe a language to control hardware
with input and output from digital/analog and CAN among
many things.
...
To conditionally turn the output __off__ we set it to zero
...
the case for multiple bit selection.
	...
	<statement> :=
		  <frame-id> <byte-pos> <hex2> <hex2> <hex2>
		| <declaration>
		| <rule>
		| <immediate>
...
currently, only output.
	...
	DEF_OUT = 1 ? (0x218[18] == 1)
	DEF_OUT = 0 ? (0x218[18] == 0)
...
The interpretation is coded as:
	...
	DEF_OUT = 1 ? (0x218[18] == 1)
	DEF_OUT = 1 ? (0x218[19] == 1)
	
	DEF_OUT = 0 ? (0x218[18] == 0)
	DEF_OUT = 0 ? (0x218[19] == 0)
...
Is coded as
    ...
    DEF_OUT = 1 ? (0x218[17] == 1)
    DEF_OUT = 0 ? (0x218[17] == 0)
	
    DEF_OUT = 1 ? (0x219[23] == 1)
    DEF_OUT = 1 ? (0x219[24] == 1)
    DEF_OUT = 0 ? (0x218[23] == 0)	
