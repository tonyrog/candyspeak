...
-define(SLT,'MC_SLT').
-define(FLDI,'MC_FLDI'). -define(FLDIS,'MC_FLDIS').
-define(JZ16,'MC_JZ16'). -define(JMP16,'MC_JMP16').
-define(ZERO,'MC_ZERO'). -define(ONE,'MC_ONE').
...
outputs() ->
    ...
    %% And whether the answer is SIGNED, which decides the C type of a local it
    %% is stored in. `index_t r = -1; if (r < 0)` is never true.
    put(retsgn, maps:from_list(
		  [{N, signed_ct(atom_to_list(CT))} || {word, N, _, CT, _} <- Words] ++
		  [{N, signed_ct(atom_to_list(R))}  || {native, N, _, R} <- Nats] ++
		  [{N, signed_ct(atom_to_list(R))}  || {pure, N, _, R} <- Terms])),
    BC = bcfile(Words, Nats, Sts, Bfs, Arrs),
    [{?PH, pfile(Words, Arrs)}, {?CH, cfile(Words)},
     {?BCH, BC}, {?WOUT, woutfile()}].

%% THE INTERMEDIATE, as terms. Jumps are symbolic here and labels take no
%% space, which is the form an optimiser would work on -- so this is what to
%% read when a word does not do what it says, and what to hold a peephole pass
%% against when there is one. Not compiled by anything.
woutfile() ->
    Items = get(wout),
    Starts = get(wstarts) ++ [{'$end', length(Items)}],
    ["%% generated by utils/gen_words.erl from utils/words.terms -- do not edit\n"
     "%%\n"
     "%% The token stream BEFORE layout: {jz,L} and {jmp,L} name a label,\n"
     "%% {label,L} marks one and occupies nothing. gen/csp_words_bc.h is this\n"
     "%% after relaxation -- every jump given the shortest form that reaches.\n\n",
     [[io_lib:format("{word, ~p, [\n", [N]),
       string:join([["  ", string:join([io_lib:format("~p", [pretty(X)]) || X <- G], ", ")]
		    || G <- group(lists:sublist(Items, A + 1, B - A))], ",\n"),
       "\n]}.\n\n"]
      || {{N, A}, {_, B}} <- lists:zip(lists:droplast(Starts), tl(Starts))]].

%% One INSTRUCTION per line: an opcode and the operands that belong to it. A
%% flat list of bytes is what the machine wants and not what a reader does.
%% The family tag is a CHARACTER in the stream ($D, $W); printed as a term it
%% comes out as 68 and 87, which is not what anyone wants to read.
pretty({field, {T, F}}) -> {field, list_to_atom([T]), F};
pretty(X)               -> X.

group([]) -> [];
group([H | T]) ->
    {Ops, Rest} = lists:splitwith(fun(X) -> not starts(X) end, T),
    [[H | Ops] | group(Rest)].

starts(A) when is_atom(A) -> lists:prefix("MC_", atom_to_list(A));
starts({label, _})        -> true;
starts({jz, _})           -> true;
starts({jmp, _})          -> true;
starts(_)                 -> false.
...
pfile(Words, Arrs) ->
    [banner(),
     ...
     arr_accessors(Arrs),
     %% C LINKAGE for everything with a symbol. The sketch is C++ (see
     %% CandySpeak/), so a word declared without this is looked up under a
     %% mangled name and the link fails on the board and nowhere else. The
     %% inline accessors above stay outside: they have no linkage to give.
     "\n#ifdef __cplusplus\nEXTERN_C_BEGIN\n#endif\n\n",
    ...
    [case sets:is_element(Name, get(aux)) of
     ...
     io_lib:format("~s csp_~s(csp_rt_t* st~s)\n{\n",
		   [atom_to_list(CT), atom_to_list(Name),
		    [io_lib:format(", index_t ~s", [A]) || A <- Args]]),
     [io_lib:format("\t~s ~s;\n",
		    [ltype(maps:get(L, LW, 1), maps:get(L, LS, false)), L])
      || L <- Locals],
...
cstore({decl, E, F, K}, V) ->
    ["csp_decl_set_", atom_to_list(F), "(ram_decl_at(st, ", cexp(E),
     "), (uint8_t)", cexp(K), ", ", cexp(V), ")"];
cstore({decl, E, F}, V) ->
    ["csp_decl_set_", atom_to_list(F), usuf(decl, F), "(ram_decl_at(st, ",
     cexp(E), "), ", cexp(V), ")"];
cstore({instr, E, F}, V) ->
...
%% An ARRAY field: one accessor, an element index. Reached through
%% csp_decl_ref rather than the decl() macro, which names a field and not an
%% element of one.
cexp({decl, E, F, K}) ->
    ["csp_decl_get_", atom_to_list(F), "(csp_decl_ref(st, ", lists:flatten(cexp(E)),
     "), (uint8_t)", lists:flatten(cexp(K)), ")"];
cexp({decl, E, F}) ->
    case usuf(decl, F) of
	[] -> ["decl(st, ", cexp(E), ", ", atom_to_list(F), ")"];
	%% A union field has no `decl()` spelling: that macro names the member.
	_  -> ["csp_decl_get_", atom_to_list(F), "_u(csp_decl_ref(st, ",
	       cexp(E), "))"]
    end;
...
%% The family NUMBER, which the indexed opcodes carry as an immediate -- the
%% machine indexes its four-entry table with it. Same order as mc_fam[].
fnum(decl) -> 0; fnum(instr) -> 1; fnum(buf) -> 2; fnum(view) -> 3.

stag(decl)  -> {?DECLS,  ?DECLS2};
stag(instr) -> {?INSTRS, ?INSTRS2};
stag(buf)   -> {?BUFS,   ?BUFS2};
stag(view)  -> {?VIEWS,  ?VIEWS2}.

%% The MF<T>_* letter, read from layout.terms. NOT written here: it used to be
%% a literal $V in this file and a `tag(view) -> "w"` in gen_layout, and the
%% two disagreed silently the moment a `value` family joined `view`.
ftag(Rec) ->
    Fam = case Rec of
	      decl  -> decl_common;
	      instr -> instr_common;
    ...
    T = case get(ftags) of
	    undefined ->
		{ok, L} = file:consult("utils/layout.terms"),
		M = maps:from_list([{N, Tg} || {family, N, _, _, Tg} <- L]),
		put(ftags, M), M;
	    M -> M
	end,
    hd(string:uppercase(maps:get(Fam, T))).

fld_rdi(Rec, E, F, K, Env) ->
    bexp(E, Env) ++ bexp(K, Env) ++ [?FLDI, fnum(Rec), {field, {ftag(Rec), F}}].

fld_rd(Rec, Op1, Op2, E, F, Env) ->
    Op = case cells(fwidth(Rec, F)) of 1 -> Op1; 2 -> Op2 end,
    bexp(E, Env) ++ [Op, {field, {ftag(Rec), F}}].
...
%% "_u" for a field whose C type is a UNION member, nothing otherwise. A word
%% deals in numbers, so it reaches the raw pair rather than the union.
usuf(Rec, F) ->
    case fct(Rec, F) of
	CT when is_atom(CT) ->
...
%% A field's declared C type, or none.
fct(Rec, F) ->
    T = layout(),
    L = [CT || {record, R, _, _, Fs} <- T, {N, _, CT} <- Fs,
	       match_field(Rec, R, N, F)]
	++ [CT || {record, R, _, Fs} <- T, {N, _, CT} <- Fs,
		  match_field(Rec, R, N, F)],
    case L of [] -> none; [X | _] -> X end.
...
ewidth({op, Op, A, B}, Env) ->
    case lists:member(Op, [eq, ne, lt, slt]) of
...
%% A local's C type. Two cells make it uint32_t; otherwise index_t, unless
%% something SIGNED is assigned to it.
ltype(2, _)     -> "uint32_t";
ltype(_, true)  -> "int";
ltype(_, false) -> "index_t".

%% Signed if ANY assignment to it is: a negative literal, or a call to
%% something whose return type is signed. Deliberately narrow -- widening it to
%% `anything that could go negative` would make an index_t local an int, and
%% BAD_INDEX stored in an int on AVR is -1.
lsigns(Body) ->
    lists:foldl(fun({N, S}, M) -> maps:put(N, S orelse maps:get(N, M, false), M) end,
		#{}, ls(Body)).

ls({local, N, E})       -> [{N, esigned(E)} | ls(E)];
ls({set, N, E})         -> [{N, esigned(E)} | ls(E)];
ls(X) when is_tuple(X)  -> lists:append([ls(E) || E <- tuple_to_list(X)]);
ls(L) when is_list(L)   -> lists:append([ls(E) || E <- L]);
ls(_)                   -> [].

esigned({const, N}) when is_integer(N), N < 0 -> true;
esigned({call, F, _}) -> maps:get(F, get(retsgn), false);
esigned(_) -> false.
...
bcfile(Words, Nats, Sts, Bfs, Arrs) ->
    ...
    {Items0, Starts0, Frames} =
	lists:foldl(fun(W, {Acc, O, Fr}) ->
			    {B, N} = bword(W, NI),
			    {Acc ++ B, O ++ [{element(2, W), length(Acc)}],
			     Fr ++ [{element(2, W), N}]}
		    end, {[], [], []}, Words),
    %% THE LAYOUT PASS. Items are symbolic until here: a jump names a label and
    %% a label takes no space. Relaxation decides which jumps need the sixteen-
    %% bit form, and only then does a byte offset mean anything.
    %% PEEPHOLE, and it can only run here: shortening the stream moves every
    %% branch after it, which is exactly what symbolic labels make harmless.
    {Items, Starts} = peephole(Items0, Starts0),
    put(wout, Items), put(wstarts, Starts),
    {ByteOf, Long} = relax(Items),
    Offs = [{N, maps:get(I, ByteOf)} || {N, I} <- Starts],
    Code = resolve(Items, ByteOf, Long),
...
tramp({word, Name, Args, CT, _}, FrMap) ->
    ...
    io_lib:format(
      ...
      "\tmc_cell_t a_[~p];\n\n"
      "~s"
      "\treturn ~scsp_word_run(st, CSP_W_~s_ENTRY, a_, ~p, ~p);\n}\n\n",
      [atom_to_list(CT), N,
       [io_lib:format(", index_t ~s", [A]) || A <- Args],
       max(length(Args), 1),
       [io_lib:format("\ta_[~p] = (mc_cell_t)~s;\n", [I, A])
	|| {I, A} <- lists:zip(lists:seq(0, length(Args) - 1), Args)],
       retcast(CT), U, length(Args), maps:get(Name, FrMap)]).

%% A cell is UNSIGNED sixteen bits. A word that answers -1 comes back as 65535
%% unless the cast goes through a signed type of the same width first -- which
%% is the whole difference between "not found" and a very large index.
retcast(CT) ->
    case signed_ct(atom_to_list(CT)) of
	true  -> io_lib:format("(~s)(int16_t)", [atom_to_list(CT)]);
	false -> io_lib:format("(~s)", [atom_to_list(CT)])
    end.

signed_ct("int")           -> true;
signed_ct("char")          -> true;
signed_ct("int8_t")        -> true;
signed_ct("int16_t")       -> true;
signed_ct("int32_t")       -> true;
signed_ct(_)               -> false.
...
%% ------------------------------------------------------------ the layout
%%
%% Jumps are symbolic until here. Choosing the sixteen-bit form makes the
%% stream LONGER, which can push another jump out of the short form's reach --
%% so this is a fixpoint: start with every jump short, grow the ones that do
%% not reach, and go round again. It terminates because growing never shrinks
%% anything, and the set of long jumps only ever gains members.
%% THE PEEPHOLE, run to a FIXPOINT. Every rule strictly shortens the stream, so
%% it terminates -- and composing beats special-casing: LIT16 0 0 needs no rule
%% of its own, it becomes LIT8 0 by one rule and ZERO by another.
%%
%% Only LITERALS are rewritten. A {cconst, X} that happens to be zero is a NAME,
%% and the static assert that it fits a byte is the only thing entitled to read
%% its value.
%%
%% It can only run here: shortening moves every branch after it, which is
%% exactly what symbolic labels make harmless.
%%
%% The word start indices move with the stream, so they are carried through.
peephole(Items, Starts) ->
    {Out, Map} = ph(Items, 0, 0, [], #{}),
    Items2 = lists:reverse(Out),
    Starts2 = [{N, maps:get(I, Map)} || {N, I} <- Starts],
    case Items2 =:= Items of
	true  -> {Items2, Starts2};
	false -> peephole(Items2, Starts2)
    end.

ph([], I, O, Acc, Map) -> {Acc, maps:put(I, O, Map)};
%% Two bytes for the two commonest values in the stream; these are one.
ph([A, 0 | T], I, O, Acc, Map) when A =:= ?LIT8 ->
    ph(T, I + 2, O + 1, [?ZERO | Acc], maps:put(I, O, Map));
ph([A, 1 | T], I, O, Acc, Map) when A =:= ?LIT8 ->
    ph(T, I + 2, O + 1, [?ONE | Acc], maps:put(I, O, Map));
%% A wide literal whose high byte is zero is a narrow one written long. No word
%% emits one today -- the constant path already picks the shortest form -- and
%% that is a fact about today's emitter, not about the next thing to touch it.
ph([A, X, 0 | T], I, O, Acc, Map) when A =:= ?LIT16, is_integer(X) ->
    ph(T, I + 3, O + 2, [X, ?LIT8 | Acc], maps:put(I, O, Map));
ph([H | T], I, O, Acc, Map) ->
    ph(T, I + 1, O + 1, [H | Acc], maps:put(I, O, Map)).

relax(Items) -> relax(Items, sets:new()).

relax(Items, Long) ->
    ByteOf = byteoffsets(Items, Long),
    Lbl = maps:from_list([{L, maps:get(I, ByteOf)}
			  || {I, {label, L}} <- lists:zip(
						 lists:seq(0, length(Items) - 1),
						 Items)]),
    Grow = sets:from_list(
	     [I || {I, It} <- lists:zip(lists:seq(0, length(Items) - 1), Items),
		   is_jump(It),
		   not sets:is_element(I, Long),
		   not fits8(disp(I, It, ByteOf, Lbl, Long))]),
    case sets:size(Grow) of
	0 -> {ByteOf, Long};
	_ -> relax(Items, sets:union(Long, Grow))
    end.
...
fits8(D) -> (D >= -128) andalso (D =< 127).

%% From the END of the jump instruction, which is where ip stands when the
%% machine adds the displacement.
disp(I, {_, L}, ByteOf, Lbl, Long) ->
    maps:get(L, Lbl) - (maps:get(I, ByteOf) + isize(I, {jz, L}, Long)).

isize(_, {label, _}, _) -> 0;
isize(I, It, Long) ->
    case is_jump(It) of
	true  -> case sets:is_element(I, Long) of true -> 3; false -> 2 end;
	false -> 1
    end.

byteoffsets(Items, Long) ->
    {M, _} = lists:foldl(
	       fun(It, {Acc, {I, B}}) ->
		       {maps:put(I, B, Acc), {I + 1, B + isize(I, It, Long)}}
	       end, {#{}, {0, 0}}, Items),
    M.

%% Symbolic items to bytes: a jump becomes its opcode and displacement, a label
%% becomes nothing.
resolve(Items, ByteOf, Long) ->
    Lbl = maps:from_list([{L, maps:get(I, ByteOf)}
			  || {I, {label, L}} <- lists:zip(
						 lists:seq(0, length(Items) - 1),
						 Items)]),
    lists:append(
      [case It of
	   {label, _} -> [];
	   {Kind, L} when Kind =:= jz; Kind =:= jmp ->
	       D = maps:get(L, Lbl) - (maps:get(I, ByteOf) + isize(I, It, Long)),
	       case sets:is_element(I, Long) of
		   false -> [short_op(Kind), D band 16#FF];
		   true  -> [long_op(Kind), D band 16#FF, (D bsr 8) band 16#FF]
	       end;
	   _ -> [It]
       end || {I, It} <- lists:zip(lists:seq(0, length(Items) - 1), Items)]).

short_op(jz) -> ?JZ;   short_op(jmp) -> ?JMP.
long_op(jz)  -> ?JZ16; long_op(jmp) -> ?JMP16.
...
bstmt({store, {Rec, E, F, K}, V}, Env)
  when Rec =:= decl; Rec =:= instr; Rec =:= buf; Rec =:= view ->
    {bexp(V, Env) ++ bexp(E, Env) ++ bexp(K, Env)
     ++ [?FLDIS, fnum(Rec), {field, {ftag(Rec), F}}], Env};
bstmt({store, T, V}, Env) ->
    {Rec, E, F} = case T of
		      {decl,  I, Fl} -> {decl,  I, Fl};
    ...
    {Code ++ bexp(E, Env) ++ [Op, {field, {ftag(Rec), F}}], Env};
bstmt({return, E}, Env) -> {bexp(E, Env) ++ [?EXIT], Env};
bstmt({do, E}, Env)     -> {bexp(E, Env) ++ [?DROP], Env};
bstmt({'if', C, T}, Env) ->
    {Tc, Env2} = bseq(T, Env),
    L = newlbl(),
    {bexp(C, Env) ++ [{jz, L}] ++ Tc ++ [{label, L}], Env2};
bstmt({'if', C, T, F}, Env) ->
    {Tc, E1} = bseq(T, Env),
    {Fc, E2} = bseq(F, E1),
    LF = newlbl(), LE = newlbl(),
    {bexp(C, Env) ++ [{jz, LF}] ++ Tc ++ [{jmp, LE}, {label, LF}]
	++ Fc ++ [{label, LE}], E2};
...
bstmt({while, C, B}, Env) ->
    ...
    Cc = bexp(C, Env),
    LT = newlbl(), LE = newlbl(),
    {[{label, LT}] ++ Cc ++ [{jz, LE}] ++ Bc ++ [{jmp, LT}, {label, LE}], Env2};
...
bstmt({switch, E, Cases, Default}, Env) ->
    ...
    %% Every arm leaves for the same place, and says so by name -- the offset
    %% is the layout pass's business, not this one's.
    LEnd = newlbl(),
    {Body, _} = lists:foldr(
		  fun({K, Bc}, {Acc, After}) ->
...
%% A fresh label. Jumps are SYMBOLIC until the stream is laid out: an offset
%% computed while emitting has to be right before anything can be moved, which
%% is why every optimisation used to be impossible and why a word that grew
%% past 127 bytes had to be split by hand.
newlbl() ->
    N = case get(lbl) of undefined -> 0; X -> X end,
    put(lbl, N + 1),
    N.

%% NEGATIVE: two's complement in a full cell. LIT8 would put -1 in one byte and
%% the machine would read 0x00FF, which is not -1 and not obviously wrong.
bexp({const, N}, _) when is_integer(N), N < 0 ->
...
%% different function.
bexp({index, E}, Env) -> bexp(E, Env) ++ [?INDEX];
%% An ARRAY field: the element index is an expression, so it goes on the stack
%% where the field id cannot.
bexp({decl, E, F, K}, Env)  -> fld_rdi(decl,  E, F, K, Env);
bexp({instr, E, F, K}, Env) -> fld_rdi(instr, E, F, K, Env);
bexp({buf, E, F, K}, Env)   -> fld_rdi(buf,   E, F, K, Env);
bexp({view, E, F, K}, Env)  -> fld_rdi(view,  E, F, K, Env);
bexp({decl, E, F}, Env)  -> fld_rd(decl,  ?DECL,  ?DECL2,  E, F, Env);
bexp({instr, E, F}, Env) -> fld_rd(instr, ?INSTR, ?INSTR2, E, F, Env);
bexp({buf, E, F}, Env)   -> fld_rd(buf,   ?BUF,   ?BUF2,   E, F, Env);
bexp({view, E, F}, Env)  -> fld_rd(view,  ?VIEW,  ?VIEW2,  E, F, Env);
...
bexp({andthen, A, B}, Env) ->
    ...
    L = newlbl(),
    bexp(A, Env) ++ [?DUP, {jz, L}, ?DROP] ++ Bc ++ [{label, L}];
...
bexp({orthen, A, B}, Env) ->
    Bc = bexp(B, Env),
    L = newlbl(),
    bexp(A, Env) ++ [?DUP, ?ZEQ, {jz, L}, ?DROP] ++ Bc ++ [{label, L}];
...
bexp({call, F, As}, Env) ->
	...
	{ok, _} ->
	    %% The CALLEE's frame size travels with the call: its entry is only
	    %% an offset, so this is the one place that knows how many locals it
	    %% wants. The machine keeps the TOP of the running frame and puts the
	    %% callee's base there -- which also tells a native how much of the
	    %% array is spoken for, should it call a word of its own.
	    Args ++ [?CALL, {wlo, F}, {whi, F}, {frame, F}];
	error ->
