...
main(["--board", Name, HFile]) ->
	...
	P ->
		...
		_ ->
		    ...
		    case kv(G, vendor, '?') of
			atmel ->
			    ok = file:write_file(HFile, avr_board_header(B, P, G)),
			    io:format("~s: ~w defines, ~w Hz~n",
				      [Name, length(kv(P, define, [])),
				       kv(P, core, 0)]);
			st ->
...
%% A BARE-METAL AVR BOARD, which is a much shorter header than the others.
%%
%% Nothing here is a register write. The two vendor headers above translate a
%% pin mux and a power word, because on those parts that translation is the work
%% -- an AVR has no mux register at all: a pin is a bit in a DDR, and
%% port/csp_avr.c works out which from the number. So what a board says here is
%% the handful of facts the port and the runtime cannot derive: the clock, the
%% console speed, and how much of 2K of RAM the code pool may claim.
%%
%% F_CPU is spelled the way avr-libc spells it, because that is the name the
%% headers and every example use, and having a second name for the same number
%% is how the two come apart.
avr_board_header(Name, P, G) ->
    U = string:uppercase(atom_to_list(Name)),
    [f("// generated by `gen_chips.erl --board ~s` -- do not edit.~n"
       "// boards/~s/~s.terms is the source.~n"
       "//~n"
       "// A bare-metal AVR: no pin mux and no power word, because the part has~n"
       "// neither. What is here is what port/csp_avr.c and the runtime cannot~n"
       "// work out for themselves.~n"
       "~n#ifndef __CSP_BOARD_~s_H__~n#define __CSP_BOARD_~s_H__~n~n",
       [Name, Name, Name, U, U]),
     f("#define CSP_BOARD_NAME  \"~s\"~n"
       "#define CSP_CHIP        ~s~n"
       "#define CSP_CORE_HZ     ~w~n"
       "#define CSP_AVR_RAM     ~w~n",
       [Name, kv(P, chip, '?'), kv(P, core, 0), kv(G, ram_kb, 0) * 1024]),
     %% avr-libc's own name for the same number. #ifndef because -DF_CPU on the
     %% command line is how a one-off build at another crystal is done.
     f("~n#ifndef F_CPU~n#define F_CPU ~wUL~n#endif~n", [kv(P, core, 0)]),
     case kv(P, console, undefined) of
	 {_Uart, Baud} ->
	     %% The unit is not stated: this part has one USART, and a board
	     %% naming uart1 would be describing a port that does not exist.
	     f("#ifndef CSP_UART_BAUD~n#define CSP_UART_BAUD ~wUL~n#endif~n",
	       [Baud]);
	 _ -> []
     end,
     case kv(P, code_budget, undefined) of
	 undefined -> [];
	 CB -> f("~n#define CSP_CODE_BUDGET ~w~n", [CB])
     end,
     case kv(P, settings_bytes, undefined) of
	 undefined -> [];
	 SB -> f("#define CSP_SETTINGS_BYTES ~w~n", [SB])
     ...
     %% The same {define, [...]} list an Arduino board carries, and read by the
     %% same helper: CSP_EXEC_ONLY, CSP_ARENA_MALLOC and the rest are properties
     %% of the BOARD, not of the toolchain that links it.
     [f("~n", []) || kv(P, define, []) =/= []],
     [arduino_define(D) || D <- kv(P, define, [])],
     f("~n#ifndef SUPPORT_REACTIVE~n#define SUPPORT_REACTIVE ~w~n#endif~n",
       [kv(P, support_reactive, 0)]),
...
check_bare(Db, {Name, P}) ->
	...
	CP ->
	    G = resolve(Db, CP),
	    case kv(G, vendor, '?') of
		%% Before the pin table is even asked for. An AVR board states no
		%% mux -- see check_avr -- and pin_table/check_irq are written
		%% around a table of alternate functions that this family does
		%% not have, so asking them anything here is a wrong answer, not
		%% a missing one.
		atmel -> check_avr(Name, P);
		Vendor ->
		    Pins = pin_table(Db, G),
...
%% A bare-metal AVR board has NO PIN MUX to check. port/csp_avr.c numbers pins
%% the way the Arduino core does -- 0..7 PORTD, 8..13 PORTB, 14..19 PORTC,
%% decoded by three comparisons at run time -- so there is nothing to translate
%% into register writes here, and a {pin, ...} line would be a fact that nothing
%% reads. Same for {irq, ...}: which INTn a pin carries is the board's
%% numbering, not the part's, which is why the Arduino branch refuses it too.
%%
%% What IS required is the clock. F_CPU is a compile-time constant on this
%% target and the UART divisor and the 1 kHz tick are both computed from it, so
%% a board that does not say gets 16 MHz assumed -- and a wrong assumption here
%% is a console that prints garbage and a timer that is off by the ratio.
check_avr(Name, P) ->
    case kv(P, core, 0) of
	0 ->
	    io:format("~s: ERROR a bare AVR board needs {core, Hz} -- it "
		      "becomes F_CPU, and the UART divisor and the tick are "
		      "both derived from it~n", [Name]), 1;
	_ -> 0
    end +
    check_avr_none(Name, "pin", [X || {pin, _, _} = X <- P],
		   "the port numbers pins the way the Arduino core does; "
		   "there is no mux to generate") +
    check_avr_none(Name, "irq", [X || {irq, _, _} = X <- P],
		   "which INTn a pin carries belongs to the board numbering, "
		   "so it is resolved at run time").

check_avr_none(_Name, _What, [], _Why) -> 0;
check_avr_none(Name, What, L, Why) ->
    io:format("~s: ERROR ~w {~s, ...} line(s) on a bare AVR board -- ~s~n",
	      [Name, length(L), What, Why]),
    1.
