...
// UDP IS IN port/csp_udp.c NOW -- csp_webots.c is the second port that wants a
// datagram socket, and Webots' snap allows one where it forbids a raw CAN
// socket. Nothing in that file was host-specific; it is BSD sockets.
...
#if !defined(CSP_EXEC_ONLY)
    {
	// Declared here rather than at the top: the queue and its drain live
	// with the stimulus parser further down, which is the only thing that
	// fills them.
	extern void injb_drain(csp_rt_t* st);

	injb_drain(st);  // stimulus frames, on the cycle a real one would land
    }
#endif
...
// `<name> <b0> <b1> ...` -- BYTES INTO A BUFFER, whatever carries it.
//
// The transport is the buffer's business, not the stimulus file's. `can 0x20 1
// 0` drives the CAN path and nothing else, so a program that moves its ground
// link from CAN to UDP -- which is one word in one declaration -- leaves every
// test describing a link it no longer has, and the failure reads as a drone
// that will not arm rather than as a test that is talking to the wrong wire.
//
// This goes in at the buffer instead: the same shadow write, RXPEND and
// buf_mark_fields a real frame gets (csp_buf_deliver), so `.rx` is raised by
// the commit exactly as it would be. What it does NOT exercise is the bus
// itself; `can` is still there for that.
//
//     Command 1 0          one frame into #buffer Command, however declared
//
// Returns 0 when the name is not a buffer, so the caller can try the other
// forms -- `Mm=20` is a WORD too.
// QUEUED, not delivered on the spot, so this arrives where a real frame does.
// cycle_input runs AFTER csp_input, and csp_can_input drains the CAN queue
// INSIDE it -- so `can 0x20 ...` lands on the next cycle while a direct write
// would land on this one. One cycle is enough to move a countdown by a tick,
// and then two spellings of the same stimulus disagree about when it happened.
#define MAX_INJ_BUF 8
static struct { index_t b; uint16_t len; uint8_t data[64]; } injb_q[MAX_INJ_BUF];
static int injb_n = 0;

// Drained at the top of csp_input, before csp_buf_input looks at anything.
void injb_drain(csp_rt_t* st)
{
    int k;

    for (k = 0; k < injb_n; k++)
	csp_buf_deliver(st, injb_q[k].b, injb_q[k].data, injb_q[k].len);
    injb_n = 0;
}

static int input_buf_frame(csp_rt_t* st, token_t* tv, size_t num, int i)
{
    tstr_t  t;
    index_t di, b;
    int n = 0;
    ...
    t.ptr = tv[i].v.str.ptr;
    t.len = tv[i].v.str.len;
    if ((di = csp_lookup_decl(st, &t)) == BAD_INDEX)
	return 0;
    if ((b = csp_buf_of_decl(st, INDEX(di))) == BAD_INDEX)
	return 0;
    if (injb_n >= MAX_INJ_BUF)
	return 0;
    for (j = i + 1; (j < (int)num) && (tv[j].t == INT) &&
		    (n < (int)sizeof(injb_q[0].data)); j++)
	injb_q[injb_n].data[n++] = (uint8_t)tv[j].v.val.i;
    if (n == 0)
	return 0;                        // a bare name is not a delivery
    injb_q[injb_n].b = b;
    injb_q[injb_n].len = (uint16_t)n;
    injb_n++;
    return j - i;
}
...
void cycle_input_values(csp_rt_t* st, token_t* tv, size_t num)
{
	...
	// A WORD followed by bytes is a buffer delivery -- tried before the
	// `name = value` forms below, which need an EQ and so cannot match it.
	if ((tv[i].t == WORD) && ((i+1) < (int)num) && (tv[i+1].t == INT)) {
	    int k = input_buf_frame(st, tv, num, i);
