/*
  VENDORED from cmaglie/FlashStorage v1.0.0 (https://github.com/cmaglie/FlashStorage),
  the low-level FlashClass only. Its FlashAsEEPROM layer is deliberately NOT taken:
  that layer keeps a RAM shadow of the whole emulated region (byte data[1024]) --
  because its API allows scattered writes with a deferred commit, so it must
  read-modify-write. We only ever stream the image forward and rewrite it whole,
  so we erase up front and buffer one row instead (see csp_arduino.c). Merely
  linking the library pulled in its `EEPROMClass EEPROM` global -- 1027 bytes of
  RAM we never touch, which a static constructor keeps alive through --gc-sections
  (same trap as the weak rom_* symbols). Hence this subset.

  Renamed from FlashStorage.h so it cannot collide with the installed library.
  Upstream is LGPL-2.1-or-later; LGPL-2.1 section 3 permits using the ordinary GPL
  for a copy, which is how it sits in this (GPL) project. Original notice follows.
