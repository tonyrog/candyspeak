...
fw() {  # fw <out> <csp>   -- a host firmware carrying that program as its ROM
	...
	src/csp_transport.c src/csp_console.c src/csp_states.c src/csp_flash.c port/csp_devices.c port/csp_socketcan.c port/csp_flash_host.c \
...
if ./csp -n -C -O "$D/eo_rom.c" "$D/eo.csp" >/dev/null 2>&1 &&
   gcc -DCSP_VERSION='"test"' -DCSP_ARENA_MALLOC -DCSP_EXEC_ONLY -Iinclude -Igen -Isrc \
       ...
       port/csp_devices.c port/csp_socketcan.c port/csp_flash_host.c \
...
if gcc -Iinclude -Igen -Isrc -O2 -o "$D/flash_guard" tests/flash_guard.c \
       src/csp_transport.c src/csp_console.c src/csp_states.c src/csp_flash.c src/csp_crc.c port/csp_devices.c port/csp_socketcan.c port/csp_flash_host.c \
...
PYEOF
got=$(printf '/quit\n' | repl ./csp "$D/fmt14.db")
# The firmware's own number, read from the header rather than written here: a
# format bump is a two-line change (csp.h and the images) and should not also be
# a test edit that only shows up as a failure afterwards.
romver=$(sed -n 's/^#define ROM_FORMAT_VERSION \([0-9]*\).*/\1/p' include/csp.h)
ck "a patch from another ROM format is refused, and says why" \
   "eeprom rejected: patch is ROM format 14, firmware is $romver -- clear it and re-enter" \
   "$got"
...
ubuild() {
	...
	src/csp_flash.c port/csp_devices.c port/csp_socketcan.c port/csp_flash_host.c \
	"$2.rom.c" -o "$1" >/dev/null 2>&1
}
